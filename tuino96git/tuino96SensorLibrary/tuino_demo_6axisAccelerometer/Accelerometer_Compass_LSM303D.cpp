...
void LSM303D::accelerometerLoop()
{
    getAccel(this->accel);
    while(!isMagReady());// wait for the magnetometer readings to be ready
    getMag(this->mag);  // get the magnetometer values, store them in mag
    
    for (int i=0; i<3; i++)
    {
        this->realAccel[i] = this->accel[i] / pow(2, 15) * ACCELE_SCALE;  // calculate real acceleration values, in units of g
    }
    this->heading = getHeading(this->mag);
    this->titleHeading = getTiltHeading(this->mag, this->realAccel);
}

float LSM303D::getXAxisAcceleration()
{
    getAccel(this->accel);
    this->realAccel[0] = this->accel[0] / pow(2, 15) * ACCELE_SCALE;
    return this->realAccel[0];
}

float LSM303D::getYAxisAcceleration()
{
    getAccel(this->accel);
    this->realAccel[1] = this->accel[1] / pow(2, 15) * ACCELE_SCALE;
    return this->realAccel[1];
}

float LSM303D::getZAxisAcceleration()
{
    getAccel(this->accel);
    this->realAccel[2] = this->accel[2] / pow(2, 15) * ACCELE_SCALE;
    return this->realAccel[2];
}

//The clockwise angle between the magnetic north and x-axis:
float LSM303D::getAngleMagNorthANDXAxis()
{
    getAccel(this->accel);
    while(!isMagReady());// wait for the magnetometer readings to be ready
    getMag(this->mag);  // get the magnetometer values, store them in mag
...
//The clockwise angle between the magnetic north and the projection
//of the positive x-axis in the horizontal plane:
float LSM303D::getAngleMagNorthANDXAxisHorizPlane()
{
    getAccel(this->accel);
    while(!isMagReady());// wait for the magnetometer readings to be ready
    getMag(this->mag);  // get the magnetometer values, store them in mag
